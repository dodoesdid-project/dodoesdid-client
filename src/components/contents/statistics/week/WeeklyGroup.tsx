...
import { ReactComponent as SuccessIndi } from '@assets/images/statistics/success-individual.svg';

interface IWeeklyGroup {
  now: Date;
}

const WeeklyGroup = ({ now }: IWeeklyGroup) => {
  const calendar = useCalendar(now);
  const weekDays = calendar.getWeekCalendar();
  const week = ['월', '화', '수', '목', '금', '토', '일'];

  return (
    <div className="w-full text-center bg-white rounded-2xl">
